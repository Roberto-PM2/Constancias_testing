...
@given(u'que ingreso mi usuario "{username}" y contraseña "{password}" basificación estatal')
...
@given(u'presiono el botón Identificarse basificación estatal')
...
@given(u'entro a la página de registro de basificación estatal')
def step_impl(context):
...
@when(u'completo el formulario de constancia de basificación estatal')
def step_impl(context):
    data = {
        'curp': 'CBEW050729MMCSHNA3',
        'filiacion': 'RFC123456',
...
@when(u'presiono el botón de guardar constancia basificación estatal')
def step_impl(context):
