...
@given(u'que ingreso mi usuario "{username}" y contraseña "{password}" admisión')
...
@given(u'presiono el botón Identificarse admisión')
...
@given(u'entro a la página de registro admisión')
def step_impl(context):
...
def step_impl(context):
    ...
    # Rellenar campos de "clave"
    context.driver.find_element(By.ID, 'clave-nueva').send_keys(data['clave'])


@when(u'presiono el botón de guardar constancia admisión')
def step_impl(context):
