...
@given(u'que ingreso mi usuario "{username}" y contraseña "{password}" reconocimiento')
...
@given(u'presiono el botón Identificarse reconocimiento')
...
@given(u'entro a la página de registro de reconocimiento')
def step_impl(context):
...
@when(u'completo el formulario de constancia de reconocimiento')
def step_impl(context):
    data = {
        'curp': 'CREW050729MMCSHNA6',
        'filiacion': 'RFC123456',
...
@when(u'presiono el botón de guardar constancia de reconocimiento')
def step_impl(context):
