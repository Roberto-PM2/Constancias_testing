...
@given(u'que ingreso mi usuario "{username}" y contraseña "{password}" promoción horizontal')
...
@given(u'presiono el botón Identificarse promoción horizontal')
...
@given(u'entro a la página de registro de promoción horizontal')
def step_impl(context):
...
@when(u'completo el formulario de constancia de promoción horizontal')
def step_impl(context):
    data = {
        'curp': 'CPHW050729MMCSHNA7',
        'filiacion': 'RFC123456',
...
@when(u'presiono el botón de guardar constancia de promoción horizontal')
def step_impl(context):
